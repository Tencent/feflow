...
class Generator {
  ...
  async run(name) {
    ...
    // 默认脚手架
    let generator = 'app';
    const dirPath = pathFn.join(pluginDir, name, 'generators');
    ...
    let path = pathFn.join(pluginDir, name, `${generator}/index.js`);
    if (!fs.existsSync(path)) {
      path = pathFn.join(pluginDir, name, 'generators', `${generator}/index.js`);
    }
