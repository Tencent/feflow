...
class Feflow {
    ...
    printBanner() {
        ...
        figlet.text('feflow', {
            ...
            console.log(chalk.cyan(` Feflow，当前版本${self.version}, 让开发工作流程更简单，主页：http://git.code.oa.com/feflow/feflow-cli     `));
    ...
    getUserInfo(accessToken) {
        const USER_INFO_URL = 'https://api.github.com/user?access_token=' + accessToken;
    ...
    /**
     * 用户鉴权，读取.feflow文件夹下的ACCESS_CHECK文件
     */
    *checkAuth() {
        const homeDir = osenv.home();
        ...
        const accessCheck = path.join(feflowRoot, '/ACCESS_CHECK');
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (fs.existsSync(accessCheck)) {
                try {
                    fileContent = fs.readFileSync(accessCheck, 'utf-8');
                ...
                const lastCheck = fileContent['last_check'];

                // 未失效
                if (Math.abs(timestamp - lastCheck) <= 60 * 60 * 24) {
                    isExpired = false;
                    resolve(1);
                }
            }
            ...
            if (isExpired) {
                opn(`https://github.com/login/oauth/authorize?client_id=88656f92f3780e2685fe&redirect_uri=http://api.feflow.oa.com/v1/auth/access_token&scope=user&state=${parseInt(Math.random() * 9000 + 1000)}`);
                resolve(0);
            } else {
                resolve(1);
            }
    ...
    *promptHandCopy() {
        ...
        shell.echo('请手动将http://api.feflow.oa.com/v1/users/getLoggedInUser返回的TOKEN复制下来：');

        let authData = yield inquirer.prompt([{
            type: 'input',
            name: 'token',
            message: `Feflow鉴权接口返回的TOKEN是：`
        }]);
        ...
        try {
            ...
            if (token) {
                ...
                authData['last_check'] = timestamp;

                // 更新时间
                fs.writeFileSync(accessCheck, JSON.stringify(authData));
            }

        } catch (ex) {
            console.log(ex);

            yield this.promptHandCopy();
        }
    ...
    *incrementUpgrade(moduleName, callback) {
        ...
        if (needUpgrade.length > 0) {
            ...
            if (confirmAnswer && confirmAnswer.upgrade) {
                ...
                if (installResult) {
                    shell.echo(chalk.green(`${moduleName}更新成功，即将进行下一步操作...`));
                    callback(moduleName);
                } else {
                    shell.echo(chalk.red(`${moduleName}更新失败，请重试`));
                }
            } else {
                shell.echo(chalk.red(`您取消了${moduleName}的更新`));
                callback(moduleName);
            }
        } else {
            callback(moduleName);
    ...
    configClient(parseArgs) {
        ...
        shell.echo(chalk.green('恭喜，初始化配置完成，马上运行您输入的命令！'));
    ...
    *scaffoldProject() {
        ...
        if (choices.length) {
            ...
            if (answers) {
                ...
                if (generatorName) {
                    yield self.incrementUpgrade(generatorName, self.runGenerator);
                }
            }

        } else {
            shell.echo(chalk.red('检测到你还未安装任何脚手架，请先安装后再进行项目初始化，参考文档：http://git.code.oa.com/feflow/feflow-cli/blob/master/README.md'));
        }
    }
    ...
    *execPlugin(pluginName, args) {
        ...
        if (!fs.existsSync(pluginPath)) {
            shell.echo(chalk.red(`检测到您还未安装插件feflow-plugin-${pluginName}, 请通过feflow install 进行安装后使用`));
            return;
        }
