...
class Feflow {
    ...
    loadPlugin(path, callback) {
        ...
        return hexoFs.readFile(path).then(function(script) {
    ...
    loadModuleList() {
        const packagePath = pathFn.join(feflowRoot, 'package.json');
        const pluginDir = pathFn.join(feflowRoot, 'node_modules');

        // Make sure package.json exists
        return hexoFs.exists(packagePath).then(function(exist) {
            if (!exist) return [];

            // Read package.json and find dependencies
            return hexoFs.readFile(packagePath).then(function(content) {
    ...
    loadModules() {
        ...
        return this.loadModuleList().map(function(name) {
            ...
            // Load plugins
            return self.loadPlugin(path).then(function() {
                console.info('Plugin loaded:' + name);
            }).catch(function(err) {
                console.error({err: err}, 'Plugin load failed: %s', chalk.magenta(name));
            });
        });
    }
    ...
    *pubArsDaily() {
        yield publish.pubArsDaily();
    }

    *pubArsPre() {
        yield publish.pubArsPre();
    }

    *pubArsProd() {
        yield publish.pubArsProd();
    }
