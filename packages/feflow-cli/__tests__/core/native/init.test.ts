...
describe('测试 @feflow/cli - Native init', () => {
  beforeEach(() => {
    fs.rmdirSync('.fef', {
      recursive: true,
    });
  ...
  describe('fef init 使用参数指定脚手架', () => {
    ...
    it('指定的脚手架已安装时, fef init 应使用该脚手架创建项目', () => {
      ...
      // 测试脚手架是否安装成功
      const isInstallSuccPattern = new RegExp(`templates\\n*${packageName}\\(${edition}\\)\\n*plugins\\s*\\n*No plugins have been installed\\n*`);
  ...
  describe('fef init 未使用参数指定脚手架', () => {
    it('未安装任何脚手架时, fef init 应该正常执行并且有提示信息返回', () => {
      const expectPattern = /You have not installed a template yet,  please use install command\. Guide: https:\/\/github\.com\/Tencent\/feflow\s*/;
