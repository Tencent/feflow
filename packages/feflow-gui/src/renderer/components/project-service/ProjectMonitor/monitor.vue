      ...
      <div class="content">
        ...
        <div class="desc">
          该项目未接入aegis（点击<a
            ...
            class="jump-button"
            @click="goToAegis"
          >这里</a>跳转）或者项目的.feflowrc文件中未配置，
        </div>
      </div>
    </div>
    <div
      v-if="hasAegisProject"
      class="aegis-project-wrap"
    >
      <!-- tabs-->
      <el-tabs
        v-model="activeName"
      ...
      <div
        v-show="activeName ==='pvuv'"
        class="tab-container first-label"
      >
        ...
        <div class="project-monitor-content">
          ...
          >
            <div
              ...
              name="echarts"
            >
              <chart :echart-data="echartData" />
            </div>
          </div>
          <div
            v-if="!echartData.length"
            class="no-echart-data"
          >
            该查询条件暂无pv&uv 数据
          </div>
        </div>
      ...
      <div
        v-show="activeName ==='jserror'"
        class="tab-container second-label"
      >
        <div class="project-monitor-header">
          <div class="form-item">
            <div class="form-label">
      ...
      <div
        v-show="showFooter"
        class="project-monitor-footer"
      >
        <span>查看更多数据请前往<span><a
          href="javascript:;"
          class="jump-button"
          @click="goToAegis"
        >aegis 官网</a> </span></span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async searchPvUvData() {
      ...
      const pvData = await getPvByDate(
        this.pvAegisId,
        this.time2string(this.startDate),
        this.time2string(this.endDate),
      );
      const uvData = await getUvByDate(
        this.pvAegisId,
        this.time2string(this.startDate),
        this.time2string(this.endDate),
      );
    ...
    async searchJsError() {
      ...
      const result = await getJsError(this.jsErrorAegisId, this.jsErrorStartDate);
    ...
    getYesterdayDate() {
      const date = +new Date(); // 获取当前日期时间戳
      const before = date - (1000 * 60 * 60 * 24);// 当前日期时间戳减去一天时间戳
